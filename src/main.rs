...
fn drop_fruit(
    ...
    if keys.just_pressed(KeyCode::Space) {
        if let Ok(mut cursor_fruit) = cf_query.get_single_mut() {
            ...
            commands.spawn((Fruit, FruitSize(size), FruitInfo(pos, vel), FallingFruit, MaterialMesh2dBundle {
                mesh: meshes
                    .add(shape::Circle::new(MULT * FRUIT_SIZES[size]).into())
                    .into(),
                material: materials.add(ColorMaterial::from(Color::WHITE)),
...
fn update_falling(time: Res<Time>, mut timer: ResMut<PhysicsTimer>, mut f_query: Query<(&mut FruitInfo, &mut Transform), With<FallingFruit>>) {
...
fn move_cursor(keys: Res<Input<KeyCode>>, mut c_query: Query<&mut Cursor>, mut t_query: Query<&mut Transform, With<CursorFruit>>, mut p_query: Query<&mut FruitInfo, With<CursorFruit>>) {
...
fn startup_sequence(
    ...
    create_fruits(commands, meshes, materials, &Cursor((L_WALL + R_WALL) / 2.0));
}
...
fn main() {
    App::new()
        .insert_resource(PhysicsTimer(Timer::from_seconds(1.0 / FPS, TimerMode::Repeating)))
        .add_plugins((DefaultPlugins, InitialPlugin))
        .add_systems(Update, (move_cursor, drop_fruit, update_falling))
        .run();
}
